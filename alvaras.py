...
def abrir_janela_adicionar():
    ...
    janela.columnconfigure(1, weight=1)  # Expande a segunda coluna (onde estão os campos de entrada)
    janela.rowconfigure(len(labels), weight=1)  # Expande a última linha (onde está o botão)
    ...
    for i, label in enumerate(labels):
        ...
        if label == "Tipo de Alvará":
            entrada = ttk.Combobox(janela, values=["Localização", "Vigilância", "Bombeiros", "Policial"])
        elif label == "Data de Vencimento":
            entrada = DateEntry(janela, date_pattern='dd/MM/yyyy', background="#444", foreground="white")
        elif label == "CNPJ":
    ...
    def salvar_alvara():
        valores = [e.get().strip() if isinstance(e, tk.Entry) else e.get() for e in entradas]
    ...
    btn_salvar.grid(row=len(labels), column=0, columnspan=2, padx=10, pady=10, sticky="ew")
...
def excluir_alvara():
    ...
    for item_id in selecionado:
        ...
        # Remove a formatação do CNPJ para comparação
        cnpj_sem_formatacao = "".join(filter(str.isdigit, valores[1]))
        
        dados = carregar_dados()
        dados = [d for d in dados if not (
            d["Empresa"].strip() == str(valores[0]).strip() and
            d["CNPJ"].strip() == cnpj_sem_formatacao and
            d["Prefeitura"].strip() == str(valores[2]).strip() and
            d["Tipo de Alvará"].strip() == str(valores[3]).strip() and
            d["Data de Vencimento"].strip() == str(valores[4]).strip()
        )]
...
def atualizar_lista(termo_pesquisa=None):
    ...
    dados.sort(key=lambda x: datetime.datetime.strptime(x["Data de Vencimento"], "%d/%m/%Y"))
    
    if termo_pesquisa:
        dados = [d for d in dados if any(
            termo_pesquisa in str(valor).lower() for valor in d.values()
        )]
    
    for alvara in dados:
        data_vencimento = datetime.datetime.strptime(alvara["Data de Vencimento"], "%d/%m/%Y").date()
        ...
        # Formata o CNPJ
        cnpj_formatado = formatar_cnpj(alvara["CNPJ"])
        
        tree.insert("", "end", values=(
            alvara["Empresa"], cnpj_formatado, alvara["Prefeitura"],
            alvara["Tipo de Alvará"], alvara["Data de Vencimento"]
        ), tags=(cor,))
...
def copiar_texto():
    ...
    try:
        ...
        messagebox.showinfo("Sucesso", "Código PIX copiado para a área de transferência!")
...
def abrir_janela_donate():
    ...
    btn_copiar = ttk.Button(frame, text="Copiar Código PIX", command=copiar_texto, width=20)  # Ajuste a largura do botão
    ...
    # Botão Fechar
    btn_fechar = ttk.Button(frame, text="Fechar", command=janela.destroy)
...
style.configure("Treeview", background="#444", fieldbackground="#444", foreground="white")
...
btn_excluir = ttk.Button(frame_botoes, text="Excluir Alvará", command=excluir_alvara, style="Red.TButton")
